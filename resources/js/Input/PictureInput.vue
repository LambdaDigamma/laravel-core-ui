<template>
    <div>
        ...
        <div class="flex w-full mt-1" ref="container">
            <input ref="file" type="file" :accept="accept" class="hidden" />

            <div
                v-if="!supportsUpload"
                id="upload-not-supported"
                class="flex flex-col items-center justify-center w-full h-full px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md"
            >
                <p class="text-gray-500">
                    Dein Gerät unterstützt das Hochladen von Dateien nicht.
                </p>
            </div>

            <div
                v-else-if="supportsPreview"
                class="relative w-full overflow-hidden rounded-md cursor-pointer"
                :class="className"
            >
                <div
                    id="dragndrop-target"
                    class="absolute inset-0 flex flex-col items-center justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md"
                >
                    <svg
                    ...
                    <p class="mt-1 text-sm text-center text-gray-600">
                        <button
                            type="button"
                            class="font-medium text-red-600 transition duration-150 ease-in-out cursor-pointer hover:text-red-500 focus:outline-none focus:underline"
                            @click="selectImage()"
                        >
                            Lade ein Foto hoch
                        </button>
